...
## 原创与生态差异
项目为独立原创实现，不是移植项目。Mooncakes 现有 `GCodinggo/moon-multipart` 和 `Songyz002/moon-multipart` 侧重 RFC 7578 字节流 parser/writer 与大文件传输；MoonFormData 侧重应用层命名 Schema、风险决策、API 版本兼容、一致性矩阵和回归基线。项目不依赖、复制或移植上述包，格式行为仅参考 RFC 7578、RFC 2046 和 RFC 8187 等公开规范，采用 Apache-2.0 许可证。
